...
static const HangulCombination hangul_combination_318Na = {
    N_ELEMENTS(hangul_combination_table_318Na),
...
static const HangulKeyboard hangul_keyboard_318Na = {
    ...
    "318Na",
    N_("Sebeolsik 318Na"),
    (ucschar*)hangul_keyboard_table_318Na,
    &hangul_combination_318Na,
	...
	if (hangul_is_choseong(ch)) {
		if (hic->buffer.choseong == 0) {
			...
			if (hangul_is_choseong(hangul_ic_peek(hic))) {
				choseong = hangul_combination_combine(
						hic->keyboard->combination, hic->buffer.choseong, ch);
			}
			if (choseong) {
		...
		if (hic->buffer.jungseong == 0) {
			...
			if (hangul_is_jungseong(hangul_ic_peek(hic))) {
				jungseong = hangul_combination_combine(
						hic->keyboard->combination, hic->buffer.jungseong, ch);
			}
			if (jungseong) {
		...
		if (hic->buffer.jongseong == 0) {
			...
			if (hangul_is_jongseong(hangul_ic_peek(hic))) {
				jongseong = hangul_combination_combine(
						hic->keyboard->combination, hic->buffer.jongseong, ch);
			}
			if (jongseong) {
...
hangul_ic_multikey_proc(HangulInputContext *hic, ucschar input_chr)
{
	...
	while (multi_key_table_iter->firstKey != 0)
	{
		if (input_chr == multi_key_table_iter->firstKey)
		{
			...
			if (hic->buffer.jongseong)
			{
				if (hangul_combination_combine(hic->keyboard->combination, hic->buffer.jongseong, selected_multi_key_row->secondKey))
				{
					ret_chr = selected_multi_key_row->secondKey;
				}
				else if (hangul_combination_combine(hic->keyboard->combination, hic->buffer.jongseong, selected_multi_key_row->thirdKey))
				{
